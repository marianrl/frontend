  ...
  const sortedData = orderBy
    ? [...data].sort((a, b) => {
        ...
        if (orderBy.key === 'auditDate' || orderBy.key === 'id') {
          ...
          return orderBy.key === 'id'
            ? (aValue as number) - (bValue as number)
            : (aValue as string).localeCompare(bValue as string);
        } else {
          // Si se está ordenando alfabéticamente, convertimos los valores a cadenas y utilizamos localeCompare
          const aValueAsString =
            orderBy.key === 'idTipoAuditoria'
              ? (aValue as { auditType: string }).auditType
              : (aValue as { id: number; audited: string }).audited;
          const bValueAsString =
            orderBy.key === 'idTipoAuditoria'
              ? (bValue as { auditType: string }).auditType
              : (bValue as { id: number; audited: string }).audited;
          ...
          <div className="scroll-window-wrapper">
            <h2>
              {auditTypeDetails === 'commonAuditDetails'
                ? 'Auditorias internas vigentes'
                : 'Auditorias AFIP vigentes'}
            </h2>
            <div>
              <Buttongroup>
                  ...
                  <thead className="table-header">
                    <tr>
                      ...
                      <th
                        ...
                        onClick={() => handleSort('idTipoAuditoria')}
                      >
                        Tipo de Auditoria
                        <span className="icon-right">
                          {orderBy && orderBy.key === 'idTipoAuditoria' ? (
                            orderBy.asc ? (
                              <MdOutlineArrowDropDown />
                            ) : (
                              <MdOutlineArrowDropUp />
                      ...
                      <th
                        className="fourth-child"
                        onClick={() => handleSort('idAuditado')}
                      >
                        Auditado
                        <span className="icon-right">
                          {orderBy && orderBy.key === 'idAuditado' ? (
                            orderBy.asc ? (
                              <MdOutlineArrowDropDown />
                            ) : (
                              <MdOutlineArrowDropUp />
                      ...
                      <tr key={index}>
                        ...
                        <td>
                          <div className="center">
                            <Link
                              to={`/${auditType}/${auditTypeDetails}/${user.id}`}
                            >
                              <Button
                                type="button"
                                label={
                                  isAudited(user) ? 'Auditado' : 'Ver detalle'
                                }
