...
const AuditTypeDropdown: React.FC<AuditTypeDropdownProps> = ({ onSelect, auditTypes, maxLength }) => {
    const [selectedValue, setSelectedValue] = React.useState<AuditType | null>(null);

    const handleChange = (event: SelectChangeEvent) => {
        const value = event.target.value as string;
        const selectedAuditType = auditTypes.find(auditType => auditType.auditType === value);
        ...
        <FormControl sx={{ m: 1, minWidth: 200 }}>
            ...
            >
                {auditTypes.map((auditType, index) => (
                    <MenuItem key={index} value={auditType.auditType}>{auditType.auditType}</MenuItem>
                ))}
            </Select>
        </FormControl>
    );
