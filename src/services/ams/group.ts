...
const groupService = {
    ...
    async createGroups(endpoint: string, group: Group): Promise<ApiResponse> {
        try {
            const response = await apiClient.post(`${API_BASE_URL}/${endpoint}`, group);
            return {
                data: response.data,
                status: response.status,
            };
    ...
    async updateGroup(endpoint: string, id: number, group: Group): Promise<ApiResponse> {
        try {
            const response = await apiClient.put(`${API_BASE_URL}/${endpoint}/${id}`, group);
            return {
                data: response.data,
                status: response.status,
            };
